...
1. Choose the public customer domain. Point `APP_URL` and Supabase Site URL to that exact HTTPS origin, allow `/auth/confirm`, and make the hosting audience public only as authorized by the owner. The private preview cannot receive Stripe/Resend webhooks or one-click unsubscribe requests from external providers.
