...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success)
      throw new ApiError(
        `Video must be prepared as MP4 before upload (up to ${MAX_VIDEO_SIZE_LABEL}). Refresh Reelform and choose your original file again. References must be JPG, PNG, or WebP up to 10 MB.`,
      );
