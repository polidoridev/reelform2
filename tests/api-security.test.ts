...
test("upload tokens are scoped to their owner and purpose", async () => {
  const token = await sign({
    user: "owner",
    kind: "upload",
    url: "https://storage.example/input.mp4",
    exp: Date.now() + 60000,
  });
  assert.equal(
    (await verify(token, "owner", "upload")).url,
    "https://storage.example/input.mp4",
  );
...
test("cross-origin mutations are rejected before identity lookup", async () => {
  await assert.rejects(
    authorize(
      new Request("https://reelform.example/api/generate", {
        method: "POST",
...
test("provider requests use server credentials and never automatically retry submissions", async () => {
  ...
  globalThis.fetch = async (input, init) => {
    calls++;
    assert.equal(input, "https://api.higgsfield.ai/test-model");
    assert.equal(
      new Headers(init?.headers).get("Authorization"),
      "Key test-key-id:test-key-secret-never-used-for-network",
    );
    throw new Error("simulated network timeout");
  };
  try {
    await assert.rejects(
      provider("test-model", { method: "POST" }),
      (e: unknown) => e instanceof ApiError && e.status === 504,
    );
...
test("JSON parser bounds streamed request bodies before parsing", async () => {
  await assert.rejects(
    readJson(
